...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const staticWaveform = (await findEl('demo-static-waveform'));
    ...
    const liveWaveform = (await findEl('demo-live-waveform'));
    const audioElement = (await findEl('demo-audio-player'));
    ...
    const voiceBtn = (await findEl('demo-voice-btn'));
    ...
    micSelector?.addEventListener('device-change', (e) => {
        if (micStateText)
            micStateText.textContent = `Selected ID: ${e.detail.deviceId}`;
    });
    // 10. Setup Voice Picker
    const voicePicker = (await findEl('demo-voice-picker'));
    ...
    if (voicePicker) {
        ...
        voicePicker.addEventListener('voice-change', (e) => {
            if (voicePickerState)
                voicePickerState.textContent = `Selected Voice: ${e.detail.voiceId}`;
        });
    }
    // 12. Setup Orb Demo
    const orb = (await findEl('demo-orb'));
    ...
    const syncOrbButtons = (state) => {
        if (orbBtn)
            orbBtn.textContent = `Cycle State: ${state || 'idle'}`;
